...
export default function OnboardingTargetWeightScreen({ navigation, route }: any) {
  const { t } = useTranslation();
  const [targetWeight, setTargetWeight] = useState('');
  const formData = route.params?.formData || {};
  // Inherit unit from weight screen
  const [unit, setUnit] = useState<WeightUnit>(formData.weight_unit || 'kg');

  const getWeightKg = (): number => {
    const val = parseFloat(targetWeight) || 0;
    return unit === 'kg' ? val : Math.round(val * 0.453592 * 10) / 10;
  };

  const tw = getWeightKg();
  const rawVal = parseFloat(targetWeight) || 0;
  const valid = targetWeight && !isNaN(rawVal) && (unit === 'kg'
  ...
  return (
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <Text style={styles.step}>{t('onboarding.step', { current: 6, total: 9 })}</Text>
        <Text style={styles.title}>{t('onboarding.targetWeightTitle')}</Text>
        <Text style={styles.subtitle}>{t('onboarding.targetWeightSubtitle')}</Text>

        {/* Unit Toggle */}
        <View style={styles.unitToggle}>
          <TouchableOpacity
            style={[styles.unitBtn, unit === 'kg' && styles.unitBtnActive]}
            onPress={() => { setUnit('kg'); setTargetWeight(''); }}
          >
            <Text style={[styles.unitBtnText, unit === 'kg' && styles.unitBtnTextActive]}>kg</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.unitBtn, unit === 'lbs' && styles.unitBtnActive]}
            onPress={() => { setUnit('lbs'); setTargetWeight(''); }}
          >
        ...
        <View style={styles.inputArea}>
          ...
          {targetWeight && !valid && <Text style={styles.error}>{t('onboarding.targetWeightError')}</Text>}
        </View>

        <View style={styles.footer}>
          <TouchableOpacity
            style={[styles.continueBtn, !valid && styles.continueBtnDisabled]}
            onPress={() => valid && navigation.navigate('OnboardingActivity', { formData: { ...formData, target_weight: tw } })}
            disabled={!valid}
          >
